...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Fzayne_2Enemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("zayne_2", "Zayne/Зейн")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Zaynes = 0  -- Declare and initialize a variable to track the number of zayne_2 NPCs in the game. Be careful when replacing 'zayne_2'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_zayne_2/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Zaynes = number_of_Zaynes + 1 -- увеличиваем количество zayne_2
 end

 function ENT:Think()
     if panicMusic and not panicMusic:IsPlaying() and number_of_Zaynes > 0 then
         panicMusic:Play()
         panicMusic:ChangeVolume(1, 0.1)
     end
 ...
 function ENT:OnRemove() -- уменьшение количества zayne_2 и остановка музыки, когда объект удаляется
  number_of_Zaynes = number_of_Zaynes - 1
  if panicMusic and panicMusic:IsPlaying() and number_of_Zaynes == 0 then
      panicMusic:Stop()
      panicMusic = nil
  end
 end
 ...
 local zayne_baby = Material("zayne_2/npc_zayne_baby.png")
 local zayne_face = Material("zayne_2/npc_zayne_face.png")
 local zayne_hand = Material("zayne_2/npc_zayne_hand.png")
 local zayne_sweatband = Material("zayne_2/npc_zayne_sweatband.png")
 local zayne_impress = Material("zayne_2/npc_zayne_impress.png")

 function ENT:DrawTranslucent()
     render.SetMaterial(zayne_baby)
     ...
     render.SetMaterial(zayne_face)
     render.DrawQuadEasy(pos, normal, 100, 130, color_white, 180)
     render.SetMaterial(zayne_hand)
     render.DrawQuadEasy(pos, normal, 100, 130, color_white, 180)
     render.SetMaterial(zayne_sweatband)
     render.DrawQuadEasy(pos, normal, 100, 130, color_white, 180)
     render.SetMaterial(zayne_impress)
     render.DrawQuadEasy(pos, normal, 100, 130, color_white, 180)
 end
end
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_zayne_2/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Fzayne_2Enemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Fzayne_2Enemy)
end
