...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.FderekEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("derek", "Derek/Дерек")
 ...
 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_derek/panic.mp3", SND_LOOP)
      panicMusic:Play()
 ...
 local derek_baby = Material("derek/npc_derek_baby.png")
 local derek_face = Material("derek/npc_derek_face.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_derek/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.FderekEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.FderekEnemy)
end
