...
if CLIENT then
 killicon.Add("ace", "ace/chibi-ace", color_white)
 language.Add("ace", "Ace/Эйс")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Aces = 0  -- Declare and initialize a variable to track the number of Ace NPCs in the game. Be careful when replacing 'Aces'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_ace/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Aces = number_of_Aces + 1 -- увеличиваем количество Ace
 end
 ...
 function ENT:OnRemove() -- уменьшение количества Ace и остановка музыки, когда объект удаляется
 ...
 local ace_baby = Material("ace/npc_ace_baby.png")
 local ace_face = Material("ace/npc_ace_face.png")
