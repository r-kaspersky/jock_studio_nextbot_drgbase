...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Fbryce_2Enemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 killicon.Add("bryce_2", "bryce_2/chibi-bryce", color_white)
 language.Add("bryce_2", "Bryce/Брайс")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Bryces = 0  -- Declare and initialize a variable to track the number of Bryces NPCs in the game. Be careful when replacing 'Bryces'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_bryce_2/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Bryces = number_of_Bryces + 1 -- увеличиваем количество Bryces
 end

 function ENT:Think()
     if panicMusic and not panicMusic:IsPlaying() and number_of_Bryces > 0 then
         panicMusic:Play()
         panicMusic:ChangeVolume(1, 0.1)
     end
 ...
 function ENT:OnRemove() -- уменьшение количества Bryces и остановка музыки, когда объект удаляется
  number_of_Bryces = number_of_Bryces - 1
  if panicMusic and panicMusic:IsPlaying() and number_of_Bryces == 0 then
      panicMusic:Stop()
      panicMusic = nil
  end
 end
 ...
 local bryce_baby = Material("bryce_2/npc_bryce_baby.png")
 local bryce_face = Material("bryce_2/npc_bryce_face.png")
 local bryce_hand = Material("bryce_2/npc_bryce_hand.png")

 function ENT:DrawTranslucent()
     render.SetMaterial(bryce_baby)
     ...
     render.SetMaterial(bryce_face)
     render.DrawQuadEasy(pos, normal, 100, 130, color_white, 180)
     render.SetMaterial(bryce_hand)
     render.DrawQuadEasy(pos, normal, 100, 130, color_white, 180)
 end
end
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_bryce_2/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Fbryce_2Enemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Fbryce_2Enemy)
end
