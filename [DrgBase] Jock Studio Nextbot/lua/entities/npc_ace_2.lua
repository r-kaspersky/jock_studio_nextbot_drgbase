...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Face_2Enemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 killicon.Add("ace_2", "ace_2/chibi-ace", color_white)
 language.Add("ace_2", "Ace/Эйс")
 ...
 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_ace_2/panic.mp3", SND_LOOP)
      panicMusic:Play()
 ...
 local ace_baby = Material("ace_2/npc_ace_baby.png")
 local ace_face = Material("ace_2/npc_ace_face.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_ace_2/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Face_2Enemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Face_2Enemy)
end
