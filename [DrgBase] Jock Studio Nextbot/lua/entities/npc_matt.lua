...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.FmattEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("matt", "Matt/Мэтт")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Matts = 0  -- Declare and initialize a variable to track the number of Matt NPCs in the game. Be careful when replacing 'Matt'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_matt/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Matts = number_of_Matts + 1 -- увеличиваем количество Matt
 end

 function ENT:Think()
     if panicMusic and not panicMusic:IsPlaying() and number_of_Matts > 0 then
         panicMusic:Play()
         panicMusic:ChangeVolume(1, 0.1)
     end
 ...
 function ENT:OnRemove() -- уменьшение количества matt и остановка музыки, когда объект удаляется
  number_of_Matts = number_of_Matts - 1
  if panicMusic and panicMusic:IsPlaying() and number_of_Matts == 0 then
      panicMusic:Stop()
      panicMusic = nil
  end
 end
 ...
 local matt_baby = Material("matt/npc_matt_baby.png")
 local matt_face = Material("matt/npc_matt_face.png")

 function ENT:DrawTranslucent()
     render.SetMaterial(matt_baby)
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_matt/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.FmattEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.FmattEnemy)
end
