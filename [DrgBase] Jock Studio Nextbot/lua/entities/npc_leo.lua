...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.FleoEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("leo", "Leo/Лео")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Leos = 0  -- Declare and initialize a variable to track the number of Leo NPCs in the game. Be careful when replacing 'Leos'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_leo/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Leos = number_of_Leos + 1 -- увеличиваем количество leo
 end
 ...
 function ENT:OnRemove() -- уменьшение количества leo и остановка музыки, когда объект удаляется
 ...
 local leo_baby = Material("leo/npc_leo_baby.png")
 local leo_face = Material("leo/npc_leo_face.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_leo/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.FleoEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.FleoEnemy)
end
