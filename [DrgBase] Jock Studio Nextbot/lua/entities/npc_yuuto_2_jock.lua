...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Fyuuto_2_jockEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("yuuto_2_jock", "Yuuto/Юто")
 ...
 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_yuuto_2_jock/panic.mp3", SND_LOOP)
      panicMusic:Play()
 ...
 local yuuto_baby = Material("yuuto_2_jock/npc_yuuto_baby.png")
 local yuuto_face = Material("yuuto_2_jock/npc_yuuto_face.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_yuuto_2_jock/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Fyuuto_2_jockEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Fyuuto_2_jockEnemy)
end
