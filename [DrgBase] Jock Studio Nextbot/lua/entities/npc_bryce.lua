...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.FbryceEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 killicon.Add("bryce", "bryce/chibi-bryce", color_white)
 language.Add("bryce", "Bryce/Брайс")
 ...
 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_bryce/panic.mp3", SND_LOOP)
      panicMusic:Play()
 ...
 local bryce_baby = Material("bryce/npc_bryce_baby.png")
 local bryce_face = Material("bryce/npc_bryce_face.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_bryce/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.FbryceEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.FbryceEnemy)
end
