...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.FzayneEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("zayne", "Zayne/Зейн")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Zaynes = 0  -- Declare and initialize a variable to track the number of zayne NPCs in the game. Be careful when replacing 'zayne'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_zayne/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Zaynes = number_of_Zaynes + 1 -- увеличиваем количество zayne
 end
 ...
 function ENT:OnRemove() -- уменьшение количества zayne и остановка музыки, когда объект удаляется
 ...
 local zayne_baby = Material("zayne/npc_zayne_baby.png")
 local zayne_face = Material("zayne/npc_zayne_face.png")
 local zayne_hair = Material("zayne/npc_zayne_hair.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_zayne/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.FzayneEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.FzayneEnemy)
end
