...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Favan_jockEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 killicon.Add("avan_jock", "avan_jock/chibi-avan", color_white)
 language.Add("avan_jock", "Avan/Эван")
 ...
 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_avan_jock/panic.mp3", SND_LOOP)
      panicMusic:Play()
 ...
 local avan_baby = Material("avan_jock/npc_avan_baby.png")
 local avan_face = Material("avan_jock/npc_avan_face.png")
 local avan_hand = Material("avan_jock/npc_avan_hand.png")
 local avan_glasses = Material("avan_jock/npc_avan_glasses.png")
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_avan_jock/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Favan_jockEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Favan_jockEnemy)
end
