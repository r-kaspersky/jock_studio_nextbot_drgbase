...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Fleo_2Enemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 language.Add("leo_2", "Leo/Лео")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Leos = 0  -- Declare and initialize a variable to track the number of leo_2 NPCs in the game. Be careful when replacing 'leo_2s'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_leo_2/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Leos = number_of_Leos + 1 -- увеличиваем количество leo_2
 end

 function ENT:Think()
     if panicMusic and not panicMusic:IsPlaying() and number_of_Leos > 0 then
         panicMusic:Play()
         panicMusic:ChangeVolume(1, 0.1)
     end
 ...
 function ENT:OnRemove() -- уменьшение количества leo_2 и остановка музыки, когда объект удаляется
  number_of_Leos = number_of_Leos - 1
  if panicMusic and panicMusic:IsPlaying() and number_of_Leos == 0 then
      panicMusic:Stop()
      panicMusic = nil
  end
 end
 ...
 local leo_baby = Material("leo_2/npc_leo_baby.png")
 local leo_face = Material("leo_2/npc_leo_face.png")

 function ENT:DrawTranslucent()
     render.SetMaterial(leo_baby)
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_leo_2/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Fleo_2Enemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Fleo_2Enemy)
end
