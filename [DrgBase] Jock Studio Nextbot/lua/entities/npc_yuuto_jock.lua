...
ENT.PossessionBinds = {
 ...
 onkeydown = function(self)
   self:EmitSound("")
   self:AttackForNpc()
   self:PlaySpriteAnimAndWait("attack", 1, self.Fyuuto_jockEnemy)
 end
 }},
 [IN_JUMP] = {{
 coroutine = false,
...
if CLIENT then
 killicon.Add("yuuto_jock", "yuuto_jock/chibi-yuuto", color_white)
 language.Add("yuuto_jock", "Yuuto/Юто")

 local panicMusic = nil
 local MUSIC_CUTOFF_DISTANCE = 8192
 local MUSIC_PANIC_DISTANCE = 4096
 local number_of_Yuutos = 0  -- Declare and initialize a variable to track the number of Yuutos NPCs in the game. Be careful when replacing 'Yuutos'.

 function ENT:Initialize() -- Уделите 1 час времени для повтора музыки. Я хотел убиться об стенку
  if not panicMusic then
      panicMusic = CreateSound(LocalPlayer(), "npc_yuuto_jock/panic.mp3", SND_LOOP)
      panicMusic:Play()
      panicMusic:ChangeVolume(1, 0.1)
  end
  number_of_Yuutos = number_of_Yuutos + 1 -- увеличиваем количество Yuutos
 end

 function ENT:Think()
     if panicMusic and not panicMusic:IsPlaying() and number_of_Yuutos > 0 then
         panicMusic:Play()
         panicMusic:ChangeVolume(1, 0.1)
     end
 ...
 function ENT:OnRemove() -- уменьшение количества Yuutos и остановка музыки, когда объект удаляется
  number_of_Yuutos = number_of_Yuutos - 1
  if panicMusic and panicMusic:IsPlaying() and number_of_Yuutos == 0 then
      panicMusic:Stop()
      panicMusic = nil
  end
 end
 ...
 local yuuto_baby = Material("yuuto_jock/npc_yuuto_baby.png")
 local yuuto_face = Material("yuuto_jock/npc_yuuto_face.png")

 function ENT:DrawTranslucent()
     render.SetMaterial(yuuto_baby)
...
function ENT:OnMeleeAttack(enemy)
  self:EmitSound("npc_yuuto_jock/damage.mp3")
  self:AttackForNpc()
  self:PlaySpriteAnimAndWait("attack", 1, self.Fyuuto_jockEnemy)
end
...
function ENT:OnDeath(dmg, delay, hitgroup)
  self:PlaySpriteAnimAndWait("death", 0.5, self.Fyuuto_jockEnemy)
end
